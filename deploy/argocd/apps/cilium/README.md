...
4. Vérifier l'absence d'erreurs dans les logs agent

**Defense-in-depth (policy SPIRE ingress)** : La policy `cilium-spire-ingress` inclut `fromEntities: host` car le SPIRE agent tourne en `hostNetwork: true` et son trafic vers le SPIRE server est identifié comme venant de l'entité `host`, pas du namespace `cilium-spire`.
